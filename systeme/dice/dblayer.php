...
/*utils*/
    ...
    {
        global $nom_page_courante;
        if (!isset($nom_page_courante))
        {
            $nom_page_courante=substr($_SERVER['PHP_SELF'], strlen($_SERVER['PHP_SELF'])-stripos(strrev($_SERVER['PHP_SELF']), '/'));
        }
        return $nom_page_courante;
    }
    ...
    function format_datetime_local($datetimelocal) {
        ...
        if ($p==0) $p = strrpos($datetimelocal, 'T');
        if ($p>0) {
...
//                $dim = getimagesize($cheminlocal);
    ...
    $mf_cpt_gabarit=1;
    $mf_cpt_modal=1; function mf_modal_get_num() { global $mf_cpt_modal; return ( $mf_cpt_modal-1 ); }
    $mf_script_end_modal = '';
    function recuperer_gabarit($filename, $trans, $forcer_mode_prod=false, $mode_modal=false)
    {
        if (MODE_PROD || MODE_DESIGN || $forcer_mode_prod || $filename=='main/page.html' || $filename=='main/page_connexion.html' || $filename=='main/section.html') // || $filename=='main/formulaire.html' || $filename=='main/header.html' || $filename=='main/footer.html'
        {
        ...
        else
        {
            ...
            if (USE_BOOTSTRAP)
            {
                $retour = "<div style=\"text-align: right; color: #aaa; margin-bottom: 5px; font-style: italic; font-weight: bold; font-family: Consolas,monaco,monospace;\">".htmlspecialchars($filename)." <button onclick=\"affichage_edition_gabarit_$mf_cpt_gabarit();\" style=\"padding: 2px 4px;\"> &lt; &gt; </button></div>
                       <div id=\"affichage_edition_gabarit_$mf_cpt_gabarit\" style=\"display: none;\">
                       <table style=\"width: 100%; font-family: consolas, monospace; font-size: 16px;\"><tr><td style=\"vertical-align: top; width: 100%;\">
                       <textarea style=\"width: 99%; height: 800px; margin-bottom: 10px; padding: 10px; background-color: #111; color: #fff; white-space: nowrap;\" id=\"id_edition_gabarit_$mf_cpt_gabarit\" onchange=\"maj_id_edition_gabarit_$mf_cpt_gabarit();\">".htmlspecialchars($txt)."</textarea>
                       ...
                       var xhr_id_edition_gabarit_$mf_cpt_gabarit;
                       function maj_id_edition_gabarit_$mf_cpt_gabarit() {
                           var modif = document.getElementById(\"id_edition_gabarit_$mf_cpt_gabarit\").value;
                           xhr_id_edition_gabarit_$mf_cpt_gabarit = getXMLHttpRequest();
                           xhr_id_edition_gabarit_$mf_cpt_gabarit.open(\"POST\", \"api/mf_gabarits/maj.php\", true);
                           xhr_id_edition_gabarit_$mf_cpt_gabarit.setRequestHeader(\"Content-Type\", \"application/x-www-form-urlencoded\");
                           xhr_id_edition_gabarit_$mf_cpt_gabarit.send(\"filename=\"+encodeURIComponent('$filename')+\"&contenu=\"+encodeURIComponent(modif));
                           xhr_id_edition_gabarit_$mf_cpt_gabarit.onreadystatechange = function() {
                               if ( xhr_id_edition_gabarit_$mf_cpt_gabarit.readyState == 4 && (xhr_id_edition_gabarit_$mf_cpt_gabarit.status == 200 || xhr_id_edition_gabarit_$mf_cpt_gabarit.status == 0) ) {
                                   var retour = xhr_id_edition_gabarit_$mf_cpt_gabarit.responseText;
                               }
                           };
                       }
                       function affichage_edition_gabarit_$mf_cpt_gabarit() {
                           var div = document.getElementById('affichage_edition_gabarit_$mf_cpt_gabarit');
                           var div_2 = document.getElementById('affichage_rendu_gabarit_$mf_cpt_gabarit');
                           ...
                           else
                           {
                               ...
                               document.location.href='".$fil_ariane->get_last_lien()."'
                           }
                       }
                       </script>";
            ...
            else
            {
                $retour = "<div style=\"text-align: right; color: #aaa; margin-bottom: 5px; font-style: italic; font-weight: bold; font-family: Consolas,monaco,monospace;\">".htmlspecialchars($filename)." <button onclick=\"affichage_edition_gabarit_$mf_cpt_gabarit();\" style=\"padding: 2px 4px;\"> &lt; &gt; </button></div>
                       ...
                       <textarea class=\"edition_gabarit\" id=\"id_edition_gabarit_$mf_cpt_gabarit\">".htmlspecialchars($txt)."</textarea>
        ...
        if ( ! MODE_PROD )
        {
            ...
            $infos_gabarit=[];
            $infos_gabarit[]= '    * C:\\wamp64\\www\\' . NOM_PROJET . '\\www\\' . NOM_PROJET . '\\gabarits\\' . str_replace('/', '\\', $filename);
            $liste_cles = lister_cles($trans);
            $txt = ''; foreach ($liste_cles as $i => $value) { $txt.= ($i>0 ? '                  ' : '' ) . $value . "\n"; }
            if ( $txt!='' )
            {
        ...
        if ( $nom_table=='' )
            ...
            if ( isset($lang_standard[$DB_name.'_']) && ( ( $mf_dictionnaire_db[$DB_name]['type']!='BOOL' || $format_button ) || ( $mf_dictionnaire_db[$DB_name]['type']=='BOOL' && $mode_formulaire ) ) ) //workflow ou bool
            {
                ...
                if ($format_button)
                {
                    foreach ($lang_standard[$DB_name."_"] as $key => $value)
                    {
                        $retour.='<button type="button" class="'.( $valeur_initiale==$key ? 'btn btn-primary' : 'btn btn-default' ).'" style="margin-top: 2px; margin-bottom: 2px;" id="bouton_mobile_auto_'.$num_champs_auto.'_'.$key.'" onclick="document.getElementById(\'form_dyn_'.$num_champs_auto.'\').value=\''.$key.'\'; maj_form_dyn_'.$num_champs_auto.'();">'.htmlspecialchars($value).'</button> ';
                    }
                    $retour.= '<script type="text/javascript">';
                    ...
                    $retour.= '#form_dyn_'.$num_champs_auto.' { display: none }'."\n";
                    $retour.= '</style>'."\n";
                }
            ...
            else
            {
                ...
                {
                    if ($type_input=='') $type_input = 'number';
                ...
                elseif ( $mf_dictionnaire_db[$DB_name]['type']=='DATETIME' || $mf_dictionnaire_db[$DB_name]['type']=='TIMESTAMP' )
                {
                    ...
                    var date_ = document.getElementById(\"form_dyn_{$num_champs_auto}_date\").value;
                    var time_ = document.getElementById(\"form_dyn_{$num_champs_auto}_time\").value;
                    var v = date_ + 'T' + time_;
                    document.getElementById(\"form_dyn_$num_champs_auto\").value = v;
                    maj_form_dyn_$num_champs_auto();
                    }";
                    $retour.= "function maj_form_dyn_{$num_champs_auto}_tmp_2() {
                        if (document.getElementById(\"form_dyn_{$num_champs_auto}_date\") !== document.activeElement && document.getElementById(\"form_dyn_{$num_champs_auto}_time\") !== document.activeElement)
                        {
                            var date_time = document.getElementById(\"form_dyn_{$num_champs_auto}\").value;
                            document.getElementById(\"form_dyn_{$num_champs_auto}_date\").value = date_time.substring(0, 10).replace(\"null\",\"\");
                            document.getElementById(\"form_dyn_{$num_champs_auto}_time\").value = date_time.substring(11).replace(\"null\",\"\");
                        }
                    }setInterval(maj_form_dyn_{$num_champs_auto}_tmp_2,100);";
                    $retour.= "</script>";
                }
                elseif ( $mf_dictionnaire_db[$DB_name]['type']=='TIME' )
                {
                    if ($type_input=='') $type_input = 'time';
                    $retour.= '<input '.( $mode_formulaire ? ' class="form-control mf_champ"' : ' class="form-control mf_champ"' )." onchange=\"maj_form_dyn_$num_champs_auto();$onchange_js;\" id=\"form_dyn_$num_champs_auto\" value=\"".htmlspecialchars($valeur_initiale)."\" type=\"$type_input\" class=\"form_dyn_champ_ $class\"$attributs_str>";
                }
                elseif ( $mf_dictionnaire_db[$DB_name]['type']=='BOOL' )
                {
                    if ($type_input=='') $type_input = 'checkbox';
            ...
            $retour.= "<script type=\"text/javascript\">
            var xhr_form_dyn_$num_champs_auto;
            var maj_form_dyn__{$num_champs_auto}_libre=true;
            var maj_form_dyn__{$num_champs_auto}_synchro_demandee=false;
            function maj_form_dyn_$num_champs_auto() {

                if ( maj_form_dyn__{$num_champs_auto}_libre )
                {

                    maj_form_dyn__{$num_champs_auto}_libre=false;
                    nb_actions_en_cours++;";
            ...
            $retour.= '{';
                ...
                $retour.= '{';
                    ...
                    if (!MODE_PROD) { $retour.= 'alertModal(xhr_form_dyn_'.$num_champs_auto.'.responseText);'; }
            ...
            if ( $maj_auto )
            {
                ...
                if ( $mode_pourcentage_montant_initiale!==false )
                {
                    ...
                    var pourcentage = document.getElementById(\"form_dyn_{$num_champs_auto}_p\").value;
                    var v = Math.round( $mode_pourcentage_montant_initiale * pourcentage )/100;
                    document.getElementById(\"form_dyn_$num_champs_auto\").value = v;
                    maj_form_dyn_$num_champs_auto();
                }";
                }
    ...
    function ajouter_champ_modifiable_interface($parametres)
    {
        ...
        return ajouter_champ_modifiable(
            $liste_valeurs_cle_table,
            $DB_name,
            $valeur_initiale,
            $nom_table,
            $rafraichissement_page,
            $class,
            $mode_pourcentage_montant_initiale,
            $type_input,
            $attributs,
            $maj_auto,
            $titre,
            $mode_formulaire,
            $mise_a_jour_silencieuse,
            $onchange_js
        );
    ...
    /*
     * $lang_standard[name] doit être définit, Ainsi que $lang_standard[name_] si necessaire
     */
    function ajouter_champ_session_modifiable_interface($parametres)
    {
        global $num_champs_auto, $lang_standard, $mf_dictionnaire_db, $fil_ariane, $mf_script_end;

        $name = $parametres['name'];
        $valeur_initiale = ( isset($_SESSION[PREFIXE_SESSION]['parametres'][$name]) ? $_SESSION[PREFIXE_SESSION]['parametres'][$name] : $parametres['valeur_initiale'] );
        ...
        if ( isset($lang_standard[$name.'_']) && ( ( $mf_dictionnaire_db[$name]['type']!='BOOL' || $format_button ) || ( $mf_dictionnaire_db[$name]['type']=='BOOL' && $mode_formulaire ) ) ) //workflow ou bool
        {
            ...
            $retour.= '<select '.( $mode_formulaire ? ' class="form-control'.$class.' mf_champ form_dyn_champ_"' : ' class="form-control'.$class.' mf_champ form_dyn_champ_"' ).' onchange="maj_form_dyn_'.$num_champs_auto.'();'.$onchange_js.';" id="form_dyn_'.$num_champs_auto.'" '.$attributs_str.'>'.( $option_vide ? '<option></option>' : '' );
            foreach ($lang_standard[$name.'_'] as $key => $value)
            {
                $retour.= "<option value=\"$key\"".( $valeur_initiale==$key ? ' selected="selected"' : '' ).'>'.htmlspecialchars($value).'</option>';
            }
        ...
        else
        {
            ...
            {
                $mode_innerHTML = true;
                $retour.= "<textarea placeholder=\"".htmlspecialchars(get_nom_colonne($name))."\" ".( $mode_formulaire ? ' class="form-control'.$class.' mf_champ form_dyn_champ_"' : ' class="form-control'.$class.' mf_champ form_dyn_champ_"' )." onchange=\"maj_form_dyn_$num_champs_auto();$onchange_js;\" id=\"form_dyn_$num_champs_auto\" $attributs_str rows=\"5\">".htmlspecialchars($valeur_initiale)."</textarea>";
            }
            elseif ( $mf_dictionnaire_db[$name]['type']=='INT' )
            {
                if ($type_input=='') $type_input = 'number';
                $retour.= "<input ".( $mode_formulaire ? ' class="form-control'.$class.' mf_champ form_dyn_champ_"' : ' class="form-control'.$class.' mf_champ form_dyn_champ_"' )." onchange=\"maj_form_dyn_$num_champs_auto();$onchange_js;\" id=\"form_dyn_$num_champs_auto\" value=\"".htmlspecialchars($valeur_initiale)."\" type=\"$type_input\" $attributs_str>";
            }
            ...
            elseif ( $mf_dictionnaire_db[$name]['type']=='DOUBLE' )
            {
                ...
                $retour.= '<td><input '.( $mode_formulaire ? ' class="form-control'.$class.' mf_champ form_dyn_champ_"' : ' class="form-control'.$class.' mf_champ form_dyn_champ_"' )." onchange=\"maj_form_dyn_$num_champs_auto();$onchange_js;\" id=\"form_dyn_$num_champs_auto\" value=\"".htmlspecialchars($valeur_initiale)."\" type=\"$type_input\" $attributs_str></td>";
                $retour.= '</tr></table>';
            }
            ...
            {
                $type_input = 'date';
                $retour.= "<table><tr><td><input ".( $mode_formulaire ? ' class="form-control'.$class.' mf_champ form_dyn_champ_"' : ' class="form-control'.$class.' mf_champ form_dyn_champ_"' )." onchange=\"maj_form_dyn_{$num_champs_auto}_tmp();$onchange_js;\" id=\"form_dyn_{$num_champs_auto}_date\" value=\"".format_date($valeur_initiale)."\" type=\"$type_input\" $attributs_str></td>";
                $type_input = 'time';
                ...
                $retour.= "<input ".( $mode_formulaire ? ' class="form-control'.$class.' mf_champ"' : ' class="form-control'.$class.' mf_champ"' )." onchange=\"maj_form_dyn_$num_champs_auto();$onchange_js;\" id=\"form_dyn_$num_champs_auto\" value=\"".format_datetime_local($valeur_initiale)."\" type=\"hidden\">";
                $retour.= "<script type=\"text/javascript\">";
                $retour.= "function maj_form_dyn_{$num_champs_auto}_tmp() {
                var date_ = document.getElementById(\"form_dyn_{$num_champs_auto}_date\").value;
                var time_ = document.getElementById(\"form_dyn_{$num_champs_auto}_time\").value;
            ...
            {
                if ($type_input=='') $type_input = 'checkbox';
                $retour.= '<input '.( $mode_formulaire ? ' class="form-control'.$class.' mf_champ form_dyn_champ_"' : ' class="'.$class.' mf_champ form_dyn_champ_"' )." onchange=\"maj_form_dyn_$num_champs_auto();$onchange_js;\" id=\"form_dyn_$num_champs_auto\" ".( $valeur_initiale==1 ? 'checked ' : '' )."type=\"$type_input\" $attributs_str>";
    ...
    function ajouter_champ_modifiable( $liste_valeurs_cle_table, $DB_name, $valeur_initiale, $nom_table='', $rafraichissement_page=false, $class='', $mode_pourcentage_montant_initiale = false, $type_input='', $attributs=array(), $maj_auto=true, $titre=true, $mode_formulaire=true, $mise_a_jour_silencieuse=false, $onchange_js='' )
    {
        ...
        if ( isset($lang_standard[$DB_name."_"]) && ($mf_dictionnaire_db[$DB_name]['type']!='BOOL' || $format_button) ) //workflow ou bool
        {
            ...
            if ($format_button)
            {
                foreach ($lang_standard[$DB_name."_"] as $key => $value)
                {
                    $retour.='<button type="button" class="'.( $valeur_initiale==$key ? 'btn btn-primary' : 'btn btn-default' ).'" id="bouton_mobile_auto_'.$num_champs_auto.'_'.$key.'" onclick="document.getElementById(\'form_dyn_'.$num_champs_auto.'\').value=\''.$key.'\'; maj_form_dyn_'.$num_champs_auto.'();">'.htmlspecialchars($value).'</button> ';
                }
                $retour.= '<script type="text/javascript">';
                $retour.= 'setInterval(function(){ $v = document.getElementById(\'form_dyn_'.$num_champs_auto.'\').value; ';
                foreach ($lang_standard[$DB_name.'_'] as $key => $value)
        ...
        else
        {
            ...
            elseif ( $mf_dictionnaire_db[$DB_name]['type']=='DATETIME' || $mf_dictionnaire_db[$DB_name]['type']=='TIMESTAMP' )
            {
                ...
                $retour.= "function maj_form_dyn_{$num_champs_auto}_tmp_2() {
                    ...
                    {
                        var date_time = document.getElementById(\"form_dyn_{$num_champs_auto}\").value;
                        document.getElementById(\"form_dyn_{$num_champs_auto}_date\").value = date_time.substring(0, 10);
                        document.getElementById(\"form_dyn_{$num_champs_auto}_time\").value = date_time.substring(11);
                    }
                }setInterval(maj_form_dyn_{$num_champs_auto}_tmp_2,100);";
                $retour.= "</script>";
            }
        ...
        $retour.= "document.getElementById(\"form_dyn_etat_$num_champs_auto\").setAttribute('class', 'maj_dyn_etat on');
                xhr_form_dyn_$num_champs_auto = getXMLHttpRequest();
                xhr_form_dyn_$num_champs_auto.open(\"POST\", \"api/$nom_table/modifier.php\", true);
                xhr_form_dyn_$num_champs_auto.setRequestHeader(\"Content-Type\", \"application/x-www-form-urlencoded\");
                xhr_form_dyn_$num_champs_auto.send(\"$cles&$DB_name=\"+encodeURIComponent(modif));
                xhr_form_dyn_$num_champs_auto.onreadystatechange = function()
                {
                    if ( xhr_form_dyn_$num_champs_auto.readyState == 4 && (xhr_form_dyn_$num_champs_auto.status == 200 || xhr_form_dyn_$num_champs_auto.status == 0) )
                    {
                        nb_actions_en_cours--;
                        var retour = JSON.parse(xhr_form_dyn_$num_champs_auto.responseText);
                        if ( retour.code_erreur == 0 )
                        {
                            document.getElementById(\"form_dyn_etat_$num_champs_auto\").setAttribute('class', 'maj_dyn_etat');
                            document.getElementById(\"form_dyn_etat_$num_champs_auto\").setAttribute('title', '');
                            ".($rafraichissement_page ? "document.location.href='".$fil_ariane->get_last_lien()."'; " : "")."
                            actualiser_les_champs();
                        }
                        else
                        {
                            document.getElementById(\"form_dyn_etat_$num_champs_auto\").setAttribute('class', 'maj_dyn_etat ko');
                            document.getElementById(\"form_dyn_etat_$num_champs_auto\").setAttribute('title', retour.message_erreur);
                        }
                    }";
...
//         $retour.= '}';
    ...
    function get_valeur_html_maj_auto_interface($parametres)
    {
        ...
        return get_valeur_html_maj_auto(
            $liste_valeurs_cle_table,
            $DB_name,
            $valeur_initiale,
            $nom_table,
    ...
    function get_valeur_html_maj_auto($liste_valeurs_cle_table, $DB_name, $valeur, $nom_table='', $class='', $maj_auto=true, $titre=true, $mode_formulaire=true, $exe_js_additionnel = '')
    {
        ...
        if ( $titre == false ) { $mode_formulaire = false; }
        ...
        if ( isset($mf_dictionnaire_db[$DB_name]['type']) )
        {
            ...
            {
                case 'DATE': $format_date = true; break;
                case 'DATETIME': $format_date_et_heure= true; break;
            }
        }
        ...
        if ($mode_formulaire)
        {
            ...
            if (!$format_html && !$format_text) { $txt.='<p>'; }
        }
        ...
        {
            if ($format_html)
                $txt.= '<div'.( $maj_auto ? ' id="champ_auto_'.$num_champs_auto.'"' : '' ).'>'.$valeur.'</div>';
            elseif ($format_date)
        ...
        if ($mode_formulaire)
        {
            if (!$format_html && !$format_text) { $txt.='</p>'; } else { $txt.='<p></p>'; }
            $txt.='</div>';
            $txt.='</div>';
        }
        ...
        if ($maj_auto)
        {
            ...
            $gvhma_db_class[$num_champs_auto] = array( 'format_price' => $format_price, 'format_date' => $format_date, 'format_date_et_heure' => $format_date_et_heure, 'zerohidden' => $zerohidden, 'format_color' => $format_color, 'exe_js_additionnel'=>$exe_js_additionnel, 'format_text'=>$format_text );
            $num_champs_auto++;
        }
    ...
    function generer_script_maj_auto()
    {
        ...
        foreach ($champ_maj_auto as $nom_table=>$temp_1)
        {
            foreach ($temp_1 as $cles=>$temp_2)
            {
                ...
                $retour.= '                 } catch (e) {';
                    ...
                    if (!MODE_PROD) { $retour.= 'if (xhr_maj_auto_'.$num_fonction.'.responseText!="") alertModal(xhr_maj_auto_'.$num_fonction.'.responseText);'; }
                ...
                foreach ($temp_2 as $DB_name=>$liste_num_champs_auto)
                {
                    ...
                    foreach ($liste_num_champs_auto as $num_champs_auto)
                    {
                        ...
                        if ( isset($mf_dictionnaire_db[$DB_name]['spec']['checkbox']) )
                        {
                            $retour.= '             document.getElementById("form_dyn_'.$num_champs_auto.'").checked = ( retour.get.'.$DB_name.'=="1" ? true : false );';
                        }
                        else
                        {
                        ...
                        {
                            if ( $gvhma_db_class[$num_champs_auto]['format_text'] ) { $ouvertures_fonctions.='escapeHtml('; $fermetures_fonctions.=')'; }
                            if ( $gvhma_db_class[$num_champs_auto]['format_price'] ) { $ouvertures_fonctions.='function_format_price('; $fermetures_fonctions.=')'; }
                            if ( $gvhma_db_class[$num_champs_auto]['format_date'] ) { $ouvertures_fonctions.='function_format_date('; $fermetures_fonctions.=')'; }
                            if ( $gvhma_db_class[$num_champs_auto]['format_date_et_heure'] ) { $ouvertures_fonctions.='function_format_date_et_heure('; $fermetures_fonctions.=')'; }
                            if ( $gvhma_db_class[$num_champs_auto]['exe_js_additionnel']!='' ) { $exe_js_additionnel = $gvhma_db_class[$num_champs_auto]['exe_js_additionnel']; }
                        }

                        $retour.= '                 document.getElementById("champ_auto_'.$num_champs_auto.'").innerHTML = '.$ouvertures_fonctions.'retour.get.'.$DB_name.$fermetures_fonctions.';';
                        ...
                        $retour.= '             if (document.getElementById("champ_auto_liste_'.$num_champs_auto.'"))
                                                {
                                                    if (document.getElementById("champ_auto_liste_'.$num_champs_auto.'").hasChildNodes())
                                                    {
                                                        ...
                                                        {
                                                            if (collEnfants[i].id=="champ_auto_liste_'.$num_champs_auto.'_"+retour.get.'.$DB_name.')
                                                            {
        ...
        $retour = $retour!='' ? '
            ...
            window.onbeforeunload = myTest_actions_en_cours;
