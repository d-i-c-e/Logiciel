...
class Formulaire {
    ...
    function ajouter_input( $nom_colonne, $valeur, $obligatoire, $type='', $attributs=array() ) {
        ...
        $this->liste_champs[] = array('input', $type, $nom_colonne, htmlspecialchars($valeur), $obligatoire, $attributs, );
    }

    function ajouter_select( $liste, $nom_colonne, $valeur, $obligatoire, $action_js="" ) {
        $this->liste_champs[] =  array('select', $liste, $nom_colonne, htmlspecialchars($valeur), $obligatoire, $action_js );
    }

    function ajouter_select_multiple( $liste, $nom_colonne, $valeurs, $obligatoire, $action_js="" ) {
        $this->liste_champs[] =  array('select_multiple', $liste, $nom_colonne, $valeurs, $obligatoire, $action_js );
    }

    function ajouter_select_double( $liste_1, $dependances, $nom_colonne_1, $nom_colonne_2, $valeur_2, $obligatoire, $action_js='', $valeur_1=0 ) { // permet l'ajout d'un double champs lie : parent -> enfant et selection sur l'enfant
    ...
    function ajouter_textarea($nom_colonne, $valeur, $obligatoire, $class='') {
        $this->liste_champs[] =  array('textarea', '', $nom_colonne, htmlspecialchars($valeur), $obligatoire, '', $class );
    }

    function ajouter_infos($legende, $infos_text)
    {
        $this->liste_champs[] = array('infos_texte', '', $legende, $infos_text, '', '', '' );
    }
    ...
    function generer_code() {
        ...
        if ($this->action!='') $code .= ' action="'.$this->action.'"';
        if ($this->activer_fichier) $code .= ' enctype="multipart/form-data"';
        $code .= '><fieldset>';
        if ($this->titre!='')
            $code .= '<legend>'.$this->titre.'</legend>';
        if ( $this->message!='' ) $code .= '<p class="message_formulaire">'.$this->message.'</p>';
        $code .= '<ul>';
    ...
    function generer_avec_gabarit( $filename )
    {
    ...
    function generer_BS4_Forms($options = array())
    {
        ...
        if ($this->titre!='') $code .= '<h2>'.$this->titre.'</h2>';

        if ( $this->message!='' ) $code .= '<div class="alert alert-info">'.$this->message.'</div>';
        ...
        {
            $max_size = min( array( ini_get ('post_max_size'), ini_get ('upload_max_filesize') ) );
            $code.='<div class="form-group'.( $this->inline ? ' row' : '' ).'">';
            if ($this->inline)
            {
                $code.='<div class="'.( $this->inline ? 'col-sm-3' : '' ).'"></div>';
            }
            $code.='<div class="'.( $this->inline ? 'col-sm-9' : '' ).'"><div class="alert alert-info">
    ...
    private function type_sql_vers_type_input($type_sql) {
        switch ($type_sql) {
            ...
            case "TIMESTAMP": $type = "datetime-local"; break;
            case "DOUBLE": $type = "text"; break;
