...
class Menu_a_droite {
    ...
    function ajouter_bouton( $titre, $lien, $type='lien', $id='', $parametres=array() ) { // ex : $parametres = array('onload'=>'do domething', ...);
        $this->liste_titres[] = array( 'type' => $type, 'text' => $titre, 'link' => $lien, 'id' => $id, 'parametres' => $parametres);
    }

    function ajouter_bouton_page_tableau( $num_page, $courante=false )
    {
        $this->liste_titres[] = array( 'type' => 'page', 'text' => '<span class="mini">Page </span>'.$num_page, 'link' => '?page='.$num_page, 'courante' => $courante );
    }

    function ajouter_bouton_formulaire( $titre )
    {
        $this->liste_titres[] = array( 'type' => 'formulaire', 'text' => $titre );
    }

    function ajouter_bouton_deconnexion()
    {
        $this->liste_titres[] = array( 'type' => 'deconnexion' );
    }
