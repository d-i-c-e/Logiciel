...
class a_candidature_joueur_groupe_monframework extends entite_monframework
{
    ...
    public function mf_ajouter($Code_joueur, $Code_groupe, $a_candidature_joueur_groupe_Message, $a_candidature_joueur_groupe_Date_envoi, $force=false)
    {
        $code_erreur = 0;
    ...
    public function mf_ajouter_2($ligne, $force=false) // array('colonne1' => 'valeur1',  [...] )
    {
        ...
        $a_candidature_joueur_groupe_Message = (isset($ligne['a_candidature_joueur_groupe_Message'])?$ligne['a_candidature_joueur_groupe_Message']:$mf_initialisation['a_candidature_joueur_groupe_Message']);
        $a_candidature_joueur_groupe_Date_envoi = (isset($ligne['a_candidature_joueur_groupe_Date_envoi'])?$ligne['a_candidature_joueur_groupe_Date_envoi']:$mf_initialisation['a_candidature_joueur_groupe_Date_envoi']);
    ...
    public function mf_modifier($Code_joueur, $Code_groupe, $a_candidature_joueur_groupe_Message, $a_candidature_joueur_groupe_Date_envoi, $force=false)
    {
        $code_erreur = 0;
        $Code_joueur = round($Code_joueur);
        $Code_groupe = round($Code_groupe);
    ...
    public function mf_modifier_4($Code_joueur, $Code_groupe, $data, $options = array( 'cond_mysql' => array() ) ) // $data = array('colonne1' => 'valeur1', ... )
    {
        ...
        if ( count($mf_colonnes_a_modifier)>0 )
        {
            ...
            $requete = 'UPDATE ' . inst('a_candidature_joueur_groupe') . ' SET ' . enumeration($mf_colonnes_a_modifier) . " WHERE 1".( $Code_joueur!=0 ? " AND Code_joueur=$Code_joueur" : "" )."".( $Code_groupe!=0 ? " AND Code_groupe=$Code_groupe" : "" )."$argument_cond;";
            $cle = md5($requete).salt(10);
            self::$cache_db->pause($cle);
            executer_requete_mysql( $requete , true);
            if ( requete_mysqli_affected_rows()==0 )
